...
document.getElementById('dataForm').addEventListener('submit', async function(e) {
    ...
    uploadPromise.then(mediaUrl => {
        ...
        showResult('Article created successfully!', '#1877f2');
            ...
            output += `
                <div class="news-item">
                    <div class="author-info">
                        ...
                        <p> ${data.author}</p>
                        <p> ${data.createdAt ? new Date(data.createdAt.seconds * 1000).toLocaleString() : 'N/A'}</p>
                        </div>
                        </div>
...
document.getElementById('editForm').addEventListener('submit', async function(e) {
    ...
    if (!docId) {
        showResult('No document ID specified.', 'red');
        return;
    }
    ...
    uploadPromise.then(newMediaUrl => {
        ...
        showResult('Article updated successfully!', '#1877f2');
...
window.deleteData = function(docId) {
    if (!docId) {
        showResult('No document selected.', 'red');
        return;
    }

    const docRef = doc(db, "article", docId);
    deleteDoc(docRef).then(() => {
        showResult('Document deleted successfully!', '#1877f2');
        loadData();
    }).catch(error => {
        showResult('Error: ' + error.message, 'red');
    });
